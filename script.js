...
function render(pl){
  ...
  const html = pl.items.map((it,idx)=>{
    ...
    return `<div class="row ${sectionClass}" data-idx="${idx}">
      <div class="time">${escapeHtml(it.time||"")}</div>
      <div class="theme editable" contenteditable="true" data-field="theme" aria-label="Thème">${escapeHtml(it.theme||"")}</div>
      <div class="person editable" contenteditable="true" data-field="person" aria-label="Personne">${escapeHtml(it.person||"")}</div>
    </div>`;
  }).join("");
...
if(resetBtn) resetBtn.addEventListener("click",async()=>{
  ...
  if(server){render(server); try{localStorage.removeItem(PLANNING_KEY);}catch(e){} alert("Planning réinitialisé depuis le fichier serveur.");}
  else alert("Impossible de recharger planning.json depuis le serveur.");
});
...
if(pdfBtn) pdfBtn.addEventListener("click",()=>{
  ...
  doc.setFont("Helvetica","bold"); doc.setFontSize(18); doc.text("Программа встречи в будние дни",14,36);
  doc.setFont("Helvetica","normal"); doc.setFontSize(12); doc.text(currentPlanning.date||"",14,56);
